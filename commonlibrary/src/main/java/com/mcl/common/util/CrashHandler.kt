...
private constructor() : Thread.UncaughtExceptionHandler {
    ...
    private fun saveCrashInfoToFile(ex: Throwable): String? {
        ...
        try {
            ...
            // 保存文件
//            val directory = File(AppUtil.Storage.getExternalCacheDir(mContext!!), "logs")
            val directory = mContext!!.getExternalFilesDir("logs")
            (!directory?.exists()!!).yes { directory.mkdirs() }
